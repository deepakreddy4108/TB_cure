...
class _PatientNotificationsState extends State<PatientNotifications>
  ...
  void initState() {
    super.initState();
    _initAnimations();
    _initializePatientIdAndFetchNotifications();
  }
  ...
  Future<void> _initializePatientIdAndFetchNotifications() async {
    try {
      final patientId = await _storage.read(key: 'patient_id');
      if (patientId != null) {
        await fetchNotifications(patientId);
      } else {
        _showModernError('Patient ID not found in storage. Please log in again.');
      }
    } catch (e) {
      _showModernError('Failed to retrieve Patient ID: $e');
    }
  }
  ...
  Future<void> fetchNotifications(String patientId) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/patient_notifications.php?patientid=$patientId'),
      );
  ...
  Future<void> markAsRead(int notificationId) async {
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/patient_notifications.php'),
  ...
  int get unreadCount => notifications.where((n) => n['status'] == 'unread').length;
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return ModernNotificationCard(
            ...
            onMarkAsRead: notifications[index]['status'] == 'unread'
...
class _ModernNotificationCardState extends State<ModernNotificationCard>
  ...
  @override
  Widget build(BuildContext context) {
    bool isUnread = widget.notification['status'] == 'unread';
